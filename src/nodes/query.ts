 ...
 */
export class Query<A extends AdaptorTypes, T> {
   ...
   * `Query` instance with those constraints applied. How constraints are
   * defined and applied is determined by the adaptor.
   *
   * @param constraints - One or more `Constraint<A>` objects to filter or modify the query.
   * @returns A new `Query<A, T>` instance with the specified constraints applied.
   *
   * @example
   * ```typescript
   ...
   * ```
   */
  constraints(...constraints: Constraint<A>[]): Query<A, T> {
    const query = this._adaptor.query(this.ref, constraints);
    return new Query(this._adaptor, query);
  }

  /**
   * Creates a new `Query` instance by applying a transformation function to
   * the current query reference. How constraints are defined and applied is
   * determined by the adaptor.
   *
   * @param getQuery - A function that receives the current `QueryRef<A, T>` and returns a new `QueryRef<A, T>`.
   * @returns A new `Query<A, T>` instance constructed with the adapted query reference.
   *
   * @example
   * ```typescript
   ...
   * ```
   */
  query(getQuery: (q: QueryRef<A, T>) => QueryRef<A, T>): Query<A, T> {
