...
describe('QueryResolver', () => {
  ...
  describe('call()', () => {
    it('should be equivalent to resolve()', () => {
      const resolver = QueryResolver.create(adaptor, () => queryRef);
      const queryViaCall = resolver.call(docRef);
      const queryViaDirect = resolver(docRef);
      expect(queryViaCall).toBeInstanceOf(Query);
      expect(queryViaDirect).toBeInstanceOf(Query);
      // Should be equivalent
      expect(queryViaCall).toEqual(queryViaDirect);
    });
  });
